...
const Products = ({ products, onDel }: Props) => {
                        ...
                        {   
                            products.map((item: IProduct, i) => (
                                <tr key={item._id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    {/* Hiển thị ảnh */}
                                    {item.images ? (
                                        <img src={`http://localhost:4200/${item.images.image1}`} alt={item._id} className="w-32 h-32 object-cover" />
                                    ) : (
                                        <img src={`http://localhost:4200/${item.images}`} alt={item._id} className="w-32 h-32 object-cover" />
                                    )}
                                </td>
                                    <td className="px-6 py-4">
                                        {item.desc}
                                    ...
                                    <td className="flex px-6 py-4 *:text-black *:px-2 *:py-1 *:mx-1 *:rounded-md ">
                                        ...
                                        <button onClick={()=>onDel(item._id)}className='bg-red-500'>
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
