...
type Props = {
    onEdit: (categories: ICategory)=>void
  }
  const cateSchemma= Joi.object({
    name: Joi.string().required().trim()
  })
const EditCate = ({onEdit }: Props) => {
    const {id}= useParams()
        ...
        <div className='relative overflow-x-auto shadow-lg border-2 sm:rounded-lg mx-5'>
            ...
            <div className='py-2 mx-6  mt-2'>
                <form className="" onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <button type="submit" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm sm:w-auto px-5 py-2  text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Sửa</button>
                </form>
            </div>
        </div>
    )
